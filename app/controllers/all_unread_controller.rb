  ...
  def show
    ...
    @t_direct_messages=TDirectMessage.select("t_direct_messages.id,t_direct_messages.directmsg,t_direct_messages.created_at,m_users.name")
        ...
        .where("t_direct_messages.send_user_id=m_users.id and t_direct_messages.read_status=false and
            t_direct_messages.receive_user_id=?",params[:user_id]).order("t_direct_messages.created_at ASC")
    ...
    and t_direct_threads.m_user_id <> ? and t_user_workspaces.workspaceid = ? and (t_direct_messages.receive_user_id = ? or t_direct_messages.send_user_id = ?)", params[:user_id], params[:workspace_id], params[:user_id], params[:user_id]).order("t_direct_threads.created_at ASC")
    ...
    INNER JOIN m_channels ON t_group_messages.m_channel_id=m_channels.id").order("t_group_messages.created_at ASC")
