class ApplicationController < ActionController::API
  ...
  def retrieve_direct_message
      ...
      TDirectThread.joins("INNER JOIN t_direct_messages ON t_direct_messages.id = t_direct_threads.t_direct_message_id").where(
        "((t_direct_messages.receive_user_id = ? and t_direct_messages.id = ? and t_direct_threads.m_user_id <> ?) or (t_direct_messages.id = ? and t_direct_threads.m_user_id <> ?))", params[:user_id], params[:t_direct_message_id], params[:user_id], params[:t_direct_message_id], params[:user_id]).update_all(read_status: true)
