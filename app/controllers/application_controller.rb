class ApplicationController < ActionController::API
  ...
  def retrieve_direct_message
      ...
      TDirectThread.joins("INNER JOIN t_direct_messages ON t_direct_messages.id = t_direct_threads.t_direct_message_id").where(m_user_id: params[:id], receive_user_id: params[:user_id], read_status: false).update_all(read_status: true)
  ...
  def retrieve_group_message
    ...
    TUserChannel.where(channelid: params[:id], userid: params[:user_id]).update_all(message_count: 0, unread_channel_message: nil, unread_thread_message: nil)
     @userid = TUserChannel.select(:userid).find_by(created_admin: true, channelid: params[:id])
  ...
  def retrieve_group_thread
    ...
    @t_group_threads = TGroupThread.select("name, groupthreadmsg, t_group_threads.id as id, t_group_threads.created_at  as created_at,t_group_threads.m_user_id")
  ...
  def retrievehome
    ...
    @m_users = MUser.joins("INNER JOIN t_user_workspaces ON t_user_workspaces.userid = m_users.id
      ...
      .where("m_users.member_status = true and m_workspaces.id = ? and m_users.id != ?", params[:workspace_id], params[:user_id])
    ...
    @m_p_channels = MChannel.select("m_channels.id,channel_name,channel_status")
      .where("(m_channels.channel_status = true and m_channels.m_workspace_id = ?)",params[:workspace_id]).order(id: :asc)
    @direct_msgcounts = Array.new
    @m_users.each do |muser|
      @direct_count = TDirectMessage.where(send_user_id: muser.id, receive_user_id: params[:user_id], read_status: false)
